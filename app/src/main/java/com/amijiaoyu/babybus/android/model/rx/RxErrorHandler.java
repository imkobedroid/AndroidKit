    ...
    public BaseException handleError(Throwable e) throws IOException {
        ...
        if (e instanceof ApiException) {
            ...
            if (httpException.response() != null
                ...
                && !Strings.isNullOrEmpty(httpException.response().errorBody().string())) {
                //请求成功后后台服务器返回的错误json
                exception.setDisplayMessage(gson.fromJson(httpException.response().errorBody().string(), Message.class).getMessage());
            } else {
                //请求发生的http错误
